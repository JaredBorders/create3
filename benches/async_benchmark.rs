...
/// bench multi-threaded generate salt.
...
/// bench single-threaded generate salt with salt prefix.
fn generate_salt_prefix_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("single-threaded generate salt prefix", |b| {
        b.iter(|| generate_salt_prefix(&deployer, salt_prefix, prefix))
    });
}

/// bench multi-threaded generate salt with salt prefix.
fn generate_salt_prefix_multithread_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("multi-threaded generate salt prefix", |b| {
        b.iter(|| {
            generate_salt_prefix_multithread(&deployer, salt_prefix, prefix, 6)
        })
    });
}
...
criterion_group!(
    ...
    targets = generate_salt_benchmark, generate_salt_multithread_benchmark,
        generate_salt_prefix_benchmark, generate_salt_prefix_multithread_benchmark
);
